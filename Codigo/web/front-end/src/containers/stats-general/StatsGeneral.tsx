...
class StatsGeneral extends React.Component<any, any> {
	...
	componentDidMount() {
		...
		// 		label: `${user.name} | ${user.businessName} | ${user.phone}`,
		// 		value: user.uid,
		// 	}));
	...
	render() {
		...
		// 			inputAttrs={{
		// 				onChange: (event) => {
		// 					this.setState({ userId: event.target.value }, this.fetchStats);
		// 				},
		// 			}}
		// 		/>
		// 		<Spacer vertical={40} />
		...
		// 						this.setState(
		// 							{
		// 								initDate: new Date(event.target.value + "T00:00:00"),
		// 							},
		// 							this.fetchStats
		// 						);
		// 					},
		...
		// 							<br />
		// 							<Spacer vertical={10} />
		// 							Quantidade de visualizações: <strong>{item.viewCount}</strong>
		// 						</Card>
		// 				  ))
