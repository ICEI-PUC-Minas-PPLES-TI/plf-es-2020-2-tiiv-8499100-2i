...
class GlobalTranslations {
  ...
  Future<Null> setNewLanguage([String newLanguage]) async {
    ...
    if (language == '') {
      ...
      if (currentLocale.length > 2) {
        if (currentLocale[2] == "-" || currentLocale[2] == "_") {
          language = currentLocale.substring(0, 2).toLowerCase();
        }
      }
    }
    ...
    String jsonContent = await rootBundle
        .loadString("assets/languages/locale_${_locale.languageCode}.json");
  ...
  // GlobalTranslations() async {
  //   print('GlobalTranslations');
  //   await this.init();
  // }
